...
model.z = Var(model.BATTERY, model.T, within=NonNegativeReals)
model.y = Var(model.BATTERY, model.T, within=NonNegativeReals)
...
#--- RELAXED FORMULATION ---
def limits_z(model, a ,t):
    return 0 <= model.z[a,t] <= 1
def limits_y(model, a ,t):
    return 0 <= model.y[a,t] <= 1
...
'''
model.y_lim = Constraint(model.BATTERY, model.T, rule=limits_y)
model.z_lim = Constraint(model.BATTERY, model.T, rule=limits_z)
