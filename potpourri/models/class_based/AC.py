...
class AC(Basemodel):
    def __init__(self, net):
        ...
        self.line_data = self.line_data.assign(**{'Bii_data': BiiT[:trafo_start], 'Bik_data': BikT[:trafo_start], 'Gii_data': GiiT[:trafo_start], 'Gik_data': GikT[:trafo_start]})
        self.trafo_data = self.trafo_data.assign(**{'BiiT_data': BiiT[trafo_start:trafo_end], 'BikT_data': BikT[trafo_start:trafo_end], 'GiiT_data': GiiT[trafo_start:trafo_end], 'GikT_data': GikT[trafo_start:trafo_end]})
    ...
    def create_model(self):
        ...
        self.model.BiiT = Param(self.model.TRANSF, within=Reals, initialize=self.trafo_data.BiiT_data[self.model.TRANSF])
        self.model.BikT = Param(self.model.TRANSF, within=Reals, initialize=self.trafo_data.BikT_data[self.model.TRANSF])
        self.model.GiiT = Param(self.model.TRANSF, within=Reals, initialize=self.trafo_data.GiiT_data[self.model.TRANSF])
        self.model.GikT = Param(self.model.TRANSF, within=Reals, initialize=self.trafo_data.GikT_data[self.model.TRANSF])
