...
def create_model_cc_node_branch(net, scenarios, case, eps, Delta):
    ...
    hc.model.Bsc = pe.Set(
        within=hc.model.B, initialize=hc.bus_lookup[scenario_busses]
    )

    sgen_p_dict = {
        (b, sc): scenarios["sgen"][case][i, sc]
        for i, b in enumerate(hc.model.Bsc)
        for sc in hc.model.S
    }
    sgen_q_dict = {
        (b, sc): scenarios["sgen_q"][case][i, sc]
        for i, b in enumerate(hc.model.Bsc)
        for sc in hc.model.S
    }
    load_p_dict = {
        (b, sc): scenarios["load"][case][i, sc]
        for i, b in enumerate(hc.model.Bsc)
        for sc in hc.model.S
    }
    load_q_dict = {
        (b, sc): scenarios["load_q"][case][i, sc]
        for i, b in enumerate(hc.model.Bsc)
        for sc in hc.model.S
    }

    hc.model.PG_sc = pe.Param(
        hc.model.Bsc,
        hc.model.S,
        within=pe.NonNegativeReals,
        initialize=sgen_p_dict,
    )
    hc.model.QG_sc = pe.Param(
        hc.model.Bsc,
        hc.model.S,
        within=pe.NonNegativeReals,
        initialize=sgen_q_dict,
    )
    hc.model.PD_sc = pe.Param(
        hc.model.Bsc,
        hc.model.S,
        within=pe.NonNegativeReals,
        initialize=load_p_dict,
    )
    hc.model.QD_sc = pe.Param(
        hc.model.Bsc,
        hc.model.S,
        within=pe.NonNegativeReals,
        initialize=load_q_dict,
    )

    hc.model.v = pe.Var(
        hc.model.B, hc.model.S, within=pe.NonNegativeReals, initialize=1.0
    )
    hc.model.delta = pe.Var(
        hc.model.B, hc.model.S, within=pe.Reals, initialize=0.0
    )
    ...
    def M_P(model, b, sc):
        return (
            ...
            + sum(
                model.psG[g]
                for g in (model.sG - model.sGc - model.WIND_HC)
                if (g, b) in model.sGbs
            )
            + sum(model.sPGmax[g] for g in model.sGc if (g, b) in model.sGbs)
            + sum(
                model.pWmax[w] for w in model.WIND_HC if (w, b) in model.sGbs
            )
    ...
    def KCL_real_def(model, b, sc):
        kcl = (
            (model.PG_sc[b, sc] - model.PD_sc[b, sc]) if b in model.Bsc else 0
        ) + sum(model.psG[g] for g in model.sG if (g, b) in model.sGbs) + sum(
            ...
            + sum(
                model.pThv[l, sc] for l in model.TRANSF if model.AT[l, 1] == b
            )
            + sum(
                model.pTlv[l, sc] for l in model.TRANSF if model.AT[l, 2] == b
            )
            + sum(
                model.GB[s] * model.v[b, sc] ** 2
                for s in model.SHUNT
                if (b, s) in model.SHUNTbs and model.GB[s] != 0
            )
        ) <= model.Delta + (
            model.u[sc] * model.M_P[b, sc] if b in model.Bsc else 0
        )
    ...
    def KCL_reactive_def(model, b, sc):
        kcl = (
            (model.QG_sc[b, sc] - model.QD_sc[b, sc]) if b in model.Bsc else 0
        ) + sum(model.qsG[g] for g in model.sG if (g, b) in model.sGbs) + sum(
            ...
            + sum(
                model.qThv[l, sc] for l in model.TRANSF if model.AT[l, 1] == b
            )
            + sum(
                model.qTlv[l, sc] for l in model.TRANSF if model.AT[l, 2] == b
            )
            - sum(
                model.BB[s] * model.v[b, sc] ** 2
                for s in model.SHUNT
                if (b, s) in model.SHUNTbs and model.BB[s] != 0
            )
        ) <= model.Delta + (
            model.u[sc] * model.M_Q[b, sc] if b in model.Bsc else 0
        )
        if isinstance(kcl, bool):
            return pe.Constraint.Skip
        return kcl
    ...
    hc.model.KCL_real = pe.Constraint(
        hc.model.B, hc.model.S, rule=KCL_real_def
    )
    hc.model.KCL_real_neg = pe.Constraint(
        hc.model.B, hc.model.S, rule=KCL_real_def_neg
    )
    hc.model.KCL_reactive = pe.Constraint(
        hc.model.B, hc.model.S, rule=KCL_reactive_def
    )
    hc.model.KCL_reactive_neg = pe.Constraint(
        hc.model.B, hc.model.S, rule=KCL_reactive_def_neg
    )

    # --- Kirchoff's voltage law on each line ---
    def KVL_real_fromend(model, l, sc):
        return model.pLfrom[l, sc] == model.Gii[l] * (
            model.v[model.A[l, 1], sc] ** 2
        ) + model.v[model.A[l, 1], sc] * model.v[model.A[l, 2], sc] * (
            model.Bik[l]
            * pe.sin(
                model.delta[model.A[l, 1], sc] - model.delta[model.A[l, 2], sc]
            )
            + model.Gik[l]
            * pe.cos(
                model.delta[model.A[l, 1], sc] - model.delta[model.A[l, 2], sc]
    ...
    def KVL_real_toend(model, l, sc):
        return model.pLto[l, sc] == model.Gii[l] * (
            model.v[model.A[l, 2], sc] ** 2
        ) + model.v[model.A[l, 1], sc] * model.v[model.A[l, 2], sc] * (
            model.Bik[l]
            * pe.sin(
                model.delta[model.A[l, 2], sc] - model.delta[model.A[l, 1], sc]
            )
            + model.Gik[l]
            * pe.cos(
                model.delta[model.A[l, 2], sc] - model.delta[model.A[l, 1], sc]
    ...
    def KVL_reactive_fromend(model, l, sc):
        return model.qLfrom[l, sc] == -model.Bii[l] * (
            model.v[model.A[l, 1], sc] ** 2
        ) + model.v[model.A[l, 1], sc] * model.v[model.A[l, 2], sc] * (
            model.Gik[l]
            * pe.sin(
    ...
    def KVL_reactive_toend(model, l, sc):
        return model.qLto[l, sc] == -model.Bii[l] * (
            model.v[model.A[l, 2], sc] ** 2
        ) + model.v[model.A[l, 1], sc] * model.v[model.A[l, 2], sc] * (
            model.Gik[l]
            * pe.sin(
    ...
    hc.model.KVL_real_from = pe.Constraint(
        hc.model.L, hc.model.S, rule=KVL_real_fromend
    )
    hc.model.KVL_real_to = pe.Constraint(
        hc.model.L, hc.model.S, rule=KVL_real_toend
    )
    hc.model.KVL_reactive_from = pe.Constraint(
        hc.model.L, hc.model.S, rule=KVL_reactive_fromend
    )
    hc.model.KVL_reactive_to = pe.Constraint(
        hc.model.L, hc.model.S, rule=KVL_reactive_toend
    )

    # --- Kirchoff's voltage law on each transformer line ---
    def KVL_real_fromendTransf(model, l, sc):
        if model.shift[l]:
            return model.pThv[l, sc] == model.GiiT[l] / model.Tap[l] ** 2 * (
                model.v[model.AT[l, 1], sc] ** 2
            ) + model.v[model.AT[l, 1], sc] * model.v[
                model.AT[l, 2], sc
            ] / model.Tap[l] * (
                model.GikT[l]
                * pe.cos(
                    model.delta[model.AT[l, 1], sc]
                    - model.delta[model.AT[l, 2], sc]
                    - model.shift[l]
                )
                + model.BikT[l]
                * pe.sin(
    ...
    def KVL_real_toendTransf(model, l, sc):
        if model.shift[l]:
            return model.pTlv[l, sc] == model.GiiT[l] * (
                model.v[model.AT[l, 2], sc] ** 2
            ) + model.v[model.AT[l, 1], sc] * model.v[
                model.AT[l, 2], sc
            ] / model.Tap[l] * (
                model.BikT[l]
    ...
    def KVL_reactive_fromendTransf(model, l, sc):
        if model.shift[l]:
            return model.qThv[l, sc] == -model.BiiT[l] / model.Tap[l] ** 2 * (
                model.v[model.AT[l, 1], sc] ** 2
            ) + model.v[model.AT[l, 1], sc] * model.v[
                model.AT[l, 2], sc
            ] / model.Tap[l] * (
    ...
    def KVL_reactive_toendTransf(model, l, sc):
        if model.shift[l]:
            return model.qTlv[l, sc] == -model.BiiT[l] * (
                model.v[model.AT[l, 2], sc] ** 2
            ) + model.v[model.AT[l, 1], sc] * model.v[
                model.AT[l, 2]
            ] / model.Tap[l] * (
    ...
    hc.model.KVL_real_fromTransf = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=KVL_real_fromendTransf
    )
    hc.model.KVL_real_toTransf = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=KVL_real_toendTransf
    )
    hc.model.KVL_reactive_fromTransf = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=KVL_reactive_fromendTransf
    )
    hc.model.KVL_reactive_toTransf = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=KVL_reactive_toendTransf
    )
    ...
    def QU_min_hc(model, w, sc):
        for g, b in model.sGbs:
            if g == w:
                return (
                    model.qsG[w]
                    >= (hc.m_qu_min * model.v[b, sc] + hc.qu_min)
                    * model.psG[w]
                )

    hc.model.QU_min_hc_constraint = pe.Constraint(
        hc.model.WIND_HC, hc.model.S, rule=QU_min_hc
    )
    ...
    def QU_max_hc(model, w, sc):
        for g, b in model.sGbs:
            if g == w:
                return (
                    model.qsG[w]
                    <= (hc.m_qu_max * model.v[b, sc] + hc.qu_max)
                    * model.psG[w]
                )

    hc.model.QU_max_hc_constraint = pe.Constraint(
        hc.model.WIND_HC, hc.model.S, rule=QU_max_hc
    )
    ...
    hc.model.v_constraint = pe.Constraint(
        hc.model.B, hc.model.S, rule=v_bounds
    )

    # --- line power limits ---
    def line_lim_from_def(model, l, sc):
        return (
            model.pLfrom[l, sc] ** 2 + model.qLfrom[l, sc] ** 2
            <= model.SLmax[l] ** 2 * model.v[model.A[l, 1], sc] ** 2
        )

    def line_lim_to_def(model, l, sc):
        return (
            model.pLto[l, sc] ** 2 + model.qLto[l, sc] ** 2
            <= model.SLmax[l] ** 2 * model.v[model.A[l, 2], sc] ** 2
        )

    hc.model.line_lim_from = pe.Constraint(
        hc.model.L, hc.model.S, rule=line_lim_from_def
    )
    hc.model.line_lim_to = pe.Constraint(
        hc.model.L, hc.model.S, rule=line_lim_to_def
    )

    # --- power flow limits on transformer lines---
    def transf_lim1_def(model, l, sc):
        return (
            model.pThv[l, sc] ** 2 + model.qThv[l, sc] ** 2
            <= model.SLmaxT[l] ** 2 * model.v[model.AT[l, 1], sc] ** 2
        )

    def transf_lim2_def(model, l, sc):
        return (
            model.pTlv[l, sc] ** 2 + model.qTlv[l, sc] ** 2
            <= model.SLmaxT[l] ** 2 * model.v[model.AT[l, 2], sc] ** 2
        )

    hc.model.transf_lim1 = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=transf_lim1_def
    )
    hc.model.transf_lim2 = pe.Constraint(
        hc.model.TRANSF, hc.model.S, rule=transf_lim2_def
    )
...
def create_model_cc_sum(net, scenarios, case, eps, Delta):
    ...
    sc = 10
    p_base_sc = sum(scenarios["sgen"][case][:, sc]) - sum(
        scenarios["load"][case][:, sc]
    )
    q_base_sc = sum(scenarios["sgen_q"][case][:, sc]) - sum(
        scenarios["load_q"][case][:, sc]
    )
    p_sc = []
    q_sc = []
    for sc in range(len(scenarios["load"][0][0])):
        p_sc.append(
            p_base_sc
            - (
                sum(scenarios["sgen"][case][:, sc])
                - sum(scenarios["load"][case][:, sc])
            )
        )
        q_sc.append(
            q_base_sc
            - (
                sum(scenarios["sgen_q"][case][:, sc])
                - sum(scenarios["load_q"][case][:, sc])
            )
        )
