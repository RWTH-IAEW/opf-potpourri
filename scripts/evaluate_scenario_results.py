...
def scenario_supply(scenarios):
    ...
    ]  # Initialize a list for each case
    scenarios["supply_sum"] = [
        [] for _ in range(n_cases)
    ]  # Initialize a list for each case
    ...
    for case in range(n_cases):
        ...
        for sc in range(n_scenarios):
            ...
            p_supply_case.append(
                p_supply
            )  # Append p_supply to the list for the current case

        scenarios["supply"][case] = np.array(p_supply_case)
        scenarios["supply_sum"][case] = np.sum(
            p_supply_case, axis=1
        )  # Sum over columns and store in 'supply_sum'


def plot_scenario_net(net, results_dir, n_cases):
    for case in range(n_cases):
        p_mw_file = os.path.join(
            results_dir, "case_" + str(case), "res_sgen", "p_mw.xlsx"
        )
        p_mw = pd.read_excel(p_mw_file, index_col=0)
        q_mvar_file = os.path.join(
            results_dir, "case_" + str(case), "res_sgen", "q_mvar.xlsx"
        )
        q_mvar = pd.read_excel(q_mvar_file, index_col=0)

        vm_pu_file = os.path.join(
            results_dir, "case_" + str(case), "res_bus", "vm_pu.xlsx"
        )
        vm_pu = pd.read_excel(vm_pu_file, index_col=0)

        ext_grid_p_mw_file = os.path.join(
            results_dir, "case_" + str(case), "res_ext_grid", "p_mw.xlsx"
        )
        ext_grid_p_mw = pd.read_excel(ext_grid_p_mw_file, index_col=0)
        ext_grid_q_mvar_file = os.path.join(
            results_dir, "case_" + str(case), "res_ext_grid", "q_mvar.xlsx"
        )
        ext_grid_q_mvar = pd.read_excel(ext_grid_q_mvar_file, index_col=0)

        line_loading_percent_file = os.path.join(
            results_dir,
            "case_" + str(case),
            "res_line",
            "loading_percent.xlsx",
        )
        line_loading_percent = pd.read_excel(
            line_loading_percent_file, index_col=0
        )
        line_pl_mw_file = os.path.join(
            results_dir, "case_" + str(case), "res_line", "pl_mw.xlsx"
        )
        line_pl_mw = pd.read_excel(line_pl_mw_file, index_col=0)
        line_ql_mvar_file = os.path.join(
            results_dir, "case_" + str(case), "res_line", "ql_mvar.xlsx"
        )
        line_ql_mvar = pd.read_excel(line_ql_mvar_file, index_col=0)

        trafo_loading_percent_file = os.path.join(
            results_dir,
            "case_" + str(case),
            "res_trafo",
            "loading_percent.xlsx",
        )
        trafo_loading_percent = pd.read_excel(
            trafo_loading_percent_file, index_col=0
        )
        ...
        for sc in range(len(p_mw)):
            ...
            try:
                ...
                print(
                    "Error plotting results of case "
                    + str(case)
                    + " scenario "
                    + str(sc)
                )
...
def std_dev_scenarios(dir):
    ...
    fig, ax = plt.subplots()
    ax.bar(
        range(len(scenarios["std_dev_sgen"][3])), scenarios["std_dev_sgen"][3]
    )
...
def std_dev_p_wind_scenarios(dir, case):
    ...
    pd.read_excel(p_mw_file, index_col=0)
...
def create_weighted_p_wind(probabilities, p_wind):
    ...
    for p_w in p_wind:
        ...
        for i, w in enumerate(weights):
            p_w_weighted = np.concatenate(
                (p_w_weighted, np.repeat(p_w[i], w.astype(int)))
            )
