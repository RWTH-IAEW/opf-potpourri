...
def max_wind_min_loss(net, n=11, **kwargs):
    ...
    for i in range(n):
        ...
        if pe.check_optimal_termination(hc.results):
            ...
            p_wind.append(
                pe.value(sum(hc.model.psG[w] for w in hc.model.WIND_HC))
            )
            p_loss.append(
                pe.value(
                    sum(
                        hc.model.pLfrom[l] + hc.model.pLto[l]
                        for l in hc.model.L
                    )
                )
            )
...
def weighted_sum(net, n=11, **kwargs):
    ...
    hc.model.w_wind = pe.Param(within=pe.Reals, initialize=0.0, mutable=True)
    hc.model.w_loss = pe.Param(within=pe.Reals, initialize=1.0, mutable=True)

    def objective_pwind_loss(model):
        return model.w_wind * sum(
            model.psG[w] for w in model.WIND_HC
        ) + model.w_loss * (
            -sum(model.pLfrom[l] + model.pLto[l] for l in model.L)
        )

    hc.model.obj_hc.deactivate()
    hc.model.OBJ_with_loss = pe.Objective(
        rule=objective_pwind_loss, sense=pe.maximize
    )
...
def pareto_front(net, n=10, **kwargs):
    ...
    p_wind_max = pe.value(hc.model.obj_hc)
    p_loss_max = pe.value(
        sum(hc.model.pLfrom[l] + hc.model.pLto[l] for l in hc.model.L)
    )
    ...
    hc.model.eps = pe.Param(
        within=pe.Reals, initialize=p_loss_min, mutable=True
    )
    hc.model.constr_loss = pe.Constraint(
        expr=sum(hc.model.pLfrom[l] + hc.model.pLto[l] for l in hc.model.L)
        <= hc.model.eps
    )
    ...
    p_wind, p_loss, nets, p_wind_opt, p_loss_opt = epsilon_constraint(
        hc, steps, p_w, p_l
    )
...
def p_wind_loss_opt(net, n=10, **kwargs):
    ...
    while step > 0.005:
        ...
        (
            ...
            p_loss_next,
            net_opt,
        ) = epsilon_constraint(hc, steps, p_w, p_l, mode="opt", ow=ow)
        ...
        if (
            ...
            and abs(step - prev_step) <= step_change_tolerance
        ):
    ...
    for i, eps in enumerate(tqdm(steps)):
        ...
        if ow:
            pf_converged = hc.results.solver.termination_condition == (
                ...
                or pe.TerminationCondition.feasible
            )
        ...
        if len(p_wind) > 1:
            p_wind_opt, p_loss_opt, p_wind_next, p_loss_next = check_slope(
                p_wind, p_loss
            )
...
def plot_pareto_front(dir):
    p_loss = pickle.load(
        open(
            dir + "sb_hv_grid_with_potential_3MW_230m/" + "p_loss_pareto_40",
            "rb",
        )
    )
    p_wind = pickle.load(
        open(
            dir + "sb_hv_grid_with_potential_3MW_230m/" + "p_wind_pareto_40",
            "rb",
        )
    )
    # plot pareto front
    ...
    fig.savefig(
        dir + "sb_hv_grid_with_potential_3MW_230m/" + "pareto_front_40.pdf",
        format="pdf",
    )
    ...
    fig.savefig(
        dir
        + "sb_hv_grid_with_potential_3MW_230m/"
        + "pareto_front_3_230_40_zoom.pdf",
        format="pdf",
    )
...
if __name__ == "__main__":
    ...
    p_wind, p_loss, nets, results, p_wind_opt, p_loss_opt, net_opt = (
        p_wind_loss_opt(net, SWmin=10.0)
    )

    p_wind, p_loss, nets, results, hc = pareto_front(
        net, n=40, tap_linear=True, SWmin=10
    )
    #
    # # plot pareto front
    # fig, ax = plt.subplots()
