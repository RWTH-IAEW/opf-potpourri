...
from src.potpourri.net_augmentation.prepare_net import (
    apply_loadcase_to_sb_net,
    upgrade_pandapower_net,
)
...
def plot_node_potential_and_hc(net, wind_pot=False):
    ...
    net.res_bus.loc[sgen_wind_bus, "p_wind_mw"] = sgen_wind_p.where(
        sgen_wind_p >= 1e-3, 0
    ).values.astype(float)

    # create marker trace with marker size scaled according to wind generation
    hosting_capacity_trace = pp.plotting.create_weighted_marker_trace(
        net,
        "res_bus",
    ...
    node_potential_trace = pp.plotting.create_weighted_marker_trace(
        net,
        "bus",
        elm_ids=net.bus.index[net.bus.node_potential_p_mw.notna()],
        column_to_plot="node_potential_p_mw",
        trace_name="Node potential",
        color="b",
        scale_legend_unit="MW",
    )

    if wind_pot:
        wind_potential_trace = pp.plotting.create_weighted_marker_trace(
            net,
            "bus",
            elm_ids=net.bus.index[net.bus.windpot_p_mw.notna()],
        ...
        fig = pp.plotting.simple_plotly(
            ...
            bus_color="black",
            additional_traces=[
                wind_potential_trace,
                node_potential_trace,
                hosting_capacity_trace,
            ],
            showlegend=True,
            auto_open=False,
        )
        ...
        fig = pp.plotting.simple_plotly(
            ...
            bus_size=3,
            bus_color="black",
            additional_traces=[node_potential_trace, hosting_capacity_trace],
            showlegend=True,
            auto_open=False,
            ext_grid_color="g",
        )
...
if __name__ == "__main__":
    ...
    for w in hc_node.model.WIND_HC:
        hc_node.model.psG[w].fix(0.0)
        hc_node.model.qsG[w].fix(0.0)
