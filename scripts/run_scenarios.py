...
colors = {"blue": "#00549F", "red": "#CC071E", "green": "#57AB27"}
...
def create_output_writer(net, time_steps, output_dir):
    ...
    ow.log_variable(
        "res_line", "pl_mw", eval_function=np.sum, eval_name="sum_p_line"
    )
    ...
    ow.log_variable(
        ...
        eval_name="sum_p_wind",
    )
    ow.log_variable(
        "res_sgen",
        ["p_mw", "y_wind"],
        eval_function=get_p_wind(ind=len(net.sgen.index)),
        eval_name="wind_p",
    )
    ow.log_variable("res_sgen", "q_mvar")
...
def run_scenarios(
    ...
    for case in cases:
        ...
        sc = 0
        write_scenario_to_model(
            hc.model, load_ind, sgen_ind, scenarios, case, sc
        )
        ...
        if not pe.check_optimal_termination(hc.results):
            print(
                "No optimal solution found for case "
                + str(case)
                + " and scenario "
                + str(sc)
            )
        ...
        pickle.dump(
            hc.net, open(output_path + "\\sc_" + str(sc) + ".pkl", "wb")
        )
        ...
        for sc in tqdm(range(1, n_scenarios)):
            ...
            hc.results.solver.termination_condition == (
...
def plot_results(nets, dir):
    ...
    unique_buses = sorted(
        list(set(bus for sublist in buses for bus in sublist))
    )

    # Load all values of net.res_sgen.p_mw where net.res_sgen.y_wind == 1 for all net in nets
    p_mw_values = [
        net.res_sgen.p_mw[net.sgen.bus.isin(unique_buses) & net.sgen.wind_hc]
        for net in nets
    ]
    ...
    fig.set_size_inches(
        (config["textbreite"] * 0.8, 0.5 * config["textbreite"])
    )
    ...
    net_min_max.bus["p_wind_max"] = 0.0
    net_min_max.bus["p_wind_min"] = 0.0
...
def plot_min_max_hc_bus(net, dir=None):
    ...
    p_wind_max_trace = pp.plotting.create_weighted_marker_trace(
        net,
        "bus",
        elm_ids=net.bus.index[net.bus.p_wind_max > 0],
        column_to_plot="p_wind_max",
        trace_name="P_wind_max",
        color=colors["blue"],
    )

    p_wind_min_trace = pp.plotting.create_weighted_marker_trace(
        net,
        "bus",
        elm_ids=net.bus.index[net.bus.p_wind_min > 0],
        column_to_plot="p_wind_min",
        trace_name="P_wind_min",
        color=colors["red"],
    )
    ...
    fig = pp.plotting.simple_plotly(
        ...
        bus_size=3,
        bus_color="black",
        additional_traces=[p_wind_max_trace, p_wind_min_trace],
        showlegend=False,
        auto_open=False,
        ext_grid_color=colors["green"],
    )
    fig.update_layout(
        {
...
if __name__ == "__main__":
    ...
    with open(
        "../potpourri/data/scenarios/sb_hv_grid_loadcases_distribution_scenarios_with_q.pkl",
        "rb",
    ) as f:
        scenarios = pickle.load(f)
    ...
    with open("../potpourri/data/windpot" + net_name + ".pkl", "rb") as f:
        net = pickle.load(f)
...
def plot_supply_task_sc(nets, scenarios, case):
    ...
    sorted_indices = np.argsort(load_sum_sc)[
        ::-1
    ...
    ax.bar(
        range(len(hc_sum)),
        hc_sum,
        label="Netzintegrationspotenzial",
        color="b",
    )
