...
def create_output_writer(net, time_steps, output_dir=None):
    ow = ts.OutputWriter(
        net, time_steps, output_path=output_dir, output_file_type=".xlsx"
    )
...
def add_wind_profile_to_net(net):
    ...
    net.sgen.profile[net.sgen.wind_hc] = net.bus["wind_profile"][
        net.sgen.bus[net.sgen.wind_hc]
    ].values
...
if __name__ == "__main__":
    ...
    absolute_values = sb.get_absolute_values(
        net, False
    )  # get absolute values for loadcases
    ...
    ts.run_time_series.control_time_step(
        controller_order, 5
    )  # write values from loadcase 'lW' to net
    ...
    output_dir = (
        "../potpourri/results/timeseries/sb_hv_grid_with_potential_3MW_230m"
    )
...
def evaluate_results(path):
    line_loading = pd.read_excel(
        path + "/res_line/loading_percent.xlsx", index_col=0
    )
    [max(loadings) for loadings in line_loading.values]

    trafo_loading = pd.read_excel(
        path + "/res_trafo/loading_percent.xlsx", index_col=0
    )
