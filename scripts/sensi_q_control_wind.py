...
def add_wind_generation_box_constraints(hc):
    ...
    hc.model.QP_hc_box_constraint_lower = pe.Constraint(
        hc.model.WIND_HC, rule=q_p_box_constraint_lower
    )
    hc.model.QP_hc_box_constraint_upper = pe.Constraint(
        hc.model.WIND_HC, rule=q_p_box_constraint_upper
    )
...
def add_wind_cos_phi_control(hc, cos_phi):
    ...
    hc.model.QP_hc_cos_phi_control = pe.Constraint(
        hc.model.WIND_HC, rule=cos_phi_control
    )
...
def plot_hc_results(hc_results, path=None):
    ...
    fig.set_size_inches(
        (config["textbreite"] * 0.7, 0.5 * config["textbreite"])
    )
    plt.show()
