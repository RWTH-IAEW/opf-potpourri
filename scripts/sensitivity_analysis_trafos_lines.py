...
def plot_cmap_loadings(
    ...
    yticks = np.around(
        np.linspace(y_ticks_max, yticks_min + step, yticks_len), decimals
    )
    xticks = np.around(
        np.linspace(xticks_min + step, x_ticks_max, xticks_len), decimals
    )
    ...
    # Adjust figure size
    fig.set_size_inches(
        (config["textbreite"] * 0.9, 0.5 * config["textbreite"])
    )
...
def calc_hc_for_loading(hc, loading_steps):
    ...
    for line_loading in tqdm(loading_steps):
        ...
        for l in hc.model.L:
            hc.model.SLmax[l] = (
                hc.line_data.loc[l, "SLmax_data"] * line_loading
            )
        ...
        for trafo_loading in loading_steps:
            ...
            for t in hc.model.TRANSF:
                hc.model.SLmaxT[t] = (
                    hc.trafo_data.loc[t, "SLmaxT_data"] * trafo_loading
                )
...
if __name__ == "__main__":
    ...
    ax = plot_cmap_loadings(
        results_list,
        step=int(step * 100),
        decimals=2,
        ticks_max=max_loading * 100,
    )
