...
def set_SLmax(self, max_loading, unit: ["percent", "MW"]):  # noqa: F821
    if unit == "percent":
        SLmax = max_loading / 100.0 * self.line_data["SLmax_data"]
    elif unit == "MW":
        self.line_data["SLmax_data"] = (
            max_loading * np.ones(len(self.model.L)) / self.baseMVA
        )
...
def store_values(hc, res):
    ...
    for b in hc.model.B:
        ...
        res["va_bus_degree"].setdefault(b, []).append(
            hc.net.res_bus.va_degree[b]
        )

    for l in hc.model.L:
        res["line_loading_%"].setdefault(l, []).append(
            hc.net.res_line.loading_percent[l]
        )
    ...
    for g in hc.model.eG:
        ...
        res["q_slack_mvar"].setdefault(g, []).append(
            hc.net.res_ext_grid.q_mvar[g]
        )
...
def plt_diff_SLmax(
    ...
    keys_to_plt=None,
    save: bool = False,
    folder="../hc_test_results/figures/",
):
    marker = ["o", "*", "+", ".", ","]
    ...
    for key, entries in res["measurements"].items():
        if key in keys_to_plt:
            ...
            if save:
                ...
                plt.savefig(
                    folder + str(key) + "_" + res["varied"]["key"] + ".png",
                    bbox_inches="tight",
                )
            else:
                plt.legend()
...
def plt_pq_wind_from_res(res, save: bool = False, folder=None):
    ...
    ax.vlines(1.0, -0.23, 0.48, color=clrs[0])  # Pmax
    ...
    if save:
        plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left")
        plt.savefig(
            folder + "pq_" + dict["varied"]["key"] + ".png",
            bbox_inches="tight",
        )
    else:
        plt.legend()
...
def plt_pq_wind_from_hc(hc):
    ...
    for w in hc.model.WIND:
        ax.plot(
            hc.model.pG[w].value / hc.model.SWmax[w].value,
            hc.model.qG[w].value / hc.model.SWmax[w].value,
            label=str(w),
            marker="o",
        )
