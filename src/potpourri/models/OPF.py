    ...
    def get_demand_real_power_data(self):
        ...
        if "controllable" not in self.net.load:
            self.demand_controllable_set = (
                None  # create empty  pyo.Set if no controllable load exist
            )
        else:
            self.demand_controllable_set = self.net.load.index[
                self.net.load.controllable.fillna(False).astype(bool)
            ]
    ...
    def add_OPF(self, **kwargs):
        ...
        Args:
            **kwargs: Forwarded to _calc_opf_parameters
                (e.g. max_loading_percent).
        """
        self._calc_opf_parameters(**kwargs)

        # controllable generation
        self.model.sGc = pyo.Set(
        ...
        self.model.sPGmax = pyo.Param(
            self.model.sGc,
            initialize=self.static_generation_data.max_p[self.model.sGc],
        )
        self.model.sPGmin = pyo.Param(
            self.model.sGc,
            initialize=self.static_generation_data.min_p[self.model.sGc],
        )
        self.model.PGmax = pyo.Param(
            self.model.G,
            initialize=self.generation_data["max_p"][self.model.G],
        )
        self.model.PGmin = pyo.Param(
            self.model.G,
            initialize=self.generation_data["min_p"][self.model.G],
        )

        # controllable loads
        self.model.Dc = pyo.Set(
            within=self.model.D, initialize=self.demand_controllable_set
        )  # controllable loads
        self.model.PDmax = pyo.Param(
            self.model.D, initialize=self.PDmax_data[self.model.D]
        )
        self.model.PDmin = pyo.Param(
            self.model.D, initialize=self.PDmin_data[self.model.D]
        )
        ...
        def _calc_tap_shift(self, tap_pos=None):
            ...
            INPUT:
                ...
                **trafo** (Dataframe) - The dataframe in
                pd_net["structure"]["trafo"] which contains transformer
                calculation values.
        ...
        def trafo_tap_discrete(model, t):
            if model.Tap_side[t]:
                ...
                return model.Tap[t] == 1 / (
                    1
                    + (model.Tap_pos[t] - model.Tap_neutral[t])
                    * model.Tap_step[t]
                )
