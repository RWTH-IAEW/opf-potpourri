...
class HC_ACOPF_multi_period(ACOPF_multi_period):
    ...
    def _calc_opf_parameters(self, SWmax=10000, SWmin=0):
        ...
        # create instance of Windpower class
        if 'windpot_p_mw' in self.net.bus:
            windpower_object = next((obj for obj in self.flexibilities if isinstance(obj, Windpower)), None)
    ...
    def add_OPF(self, **kwargs):
        ...
        if 'windpot_p_mw' in self.net.bus:
            # create instance of Windpower class
            windpower_object = next((obj for obj in self.flexibilities if isinstance(obj, Windpower)), None)
            windpower_object.get_hc_acopf_parameters(self.model,
                                                     self.net)  # Calls the get_parameters function of the Windpower class
            windpower_object.get_hc_acopf_variables(
                self.model)  # Calls the get_variables function of the Windpower class
            windpower_object.get_objective(self.model)  # Calls the objective function of the Windpower class
            windpower_object.get_constraints(self.model, self.net)  # calls constraints of the Windpower class
            windpower_object.unfix_variables(self.model)  # calls the unfix_variables function of the Windpower class
