...
class OPF_multi_period(Basemodel_multi_period):
    ...
    def _calc_opf_parameters(self, **kwargs):
        ...
        # create instance of class 'Sgens' from the 'flexibilities' list and call method 'static_generation_real_power_limits'
        ...
        # Get the object of class 'Demand' from the 'flexibilities' list and call method 'get_demand_real_power_data'
        demand_object = next((obj for obj in self.flexibilities if isinstance(obj, Demand_multi_period)), None)
        #Change: gives model instead of net, because model is needed and net is already in flexibilities
    ...
    def add_tap_changer_discrete(self):
        ...
        self.model.Tap_side = Param(self.model.TRANSF, initialize=self.trafo_data.tap_side_data[
            self.model.TRANSF])  # transformer tap side; 0: hv, 1: lv

        def trafo_tap_pos_min_max(model, tr):
            return model.Tap_pos_min[tr], model.Tap_pos[tr], model.Tap_pos_max[tr]
        ...
        def trafo_tap_discrete(model, tr):
            if model.Tap_side[tr]:
                # tap side: lv
                return model.Tap[tr] == 1 / (1 + (model.Tap_pos[tr] - model.Tap_neutral[tr]) * model.Tap_step[tr])

            return model.Tap[tr] == 1. + (model.Tap_pos[tr] - model.Tap_neutral[tr]) * model.Tap_step[tr]
