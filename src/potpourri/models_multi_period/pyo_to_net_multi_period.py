...
def _line_results_to_net(net, model, t):
    ...
    # real power on lines
    net.res_line.p_from_mw = pd.Series([model.pLfrom[l, t].value for l in net.line.index], index=net.line.index)
    net.res_line.p_from_mw *= model.baseMVA.value
    net.res_line.p_to_mw = pd.Series([model.pLto[l, t].value for l in net.line.index], index=net.line.index)
    ...
    if 'AC' in model.name:
        ...
        net.res_line.q_from_mvar = pd.Series([model.qLfrom[l, t].value for l in net.line.index], index=net.line.index)
        net.res_line.q_from_mvar *= model.baseMVA.value
        net.res_line.q_to_mvar = pd.Series([model.qLto[l, t].value for l in net.line.index], index=net.line.index)
...
def _ext_grid_results_to_net(net, model, t):
    # --- external grid ---
    net.res_ext_grid.p_mw = pd.Series(model.peG[:, t].get_values(), index=net.ext_grid.index)
...
def _generation_results_to_net(net, model, t):
    ...
    for gen, ord in net._gen_order.items():
        ...
        if 'AC' in model.name:
            ...
            net['res_' + gen].q_mvar = [qg[i, t] for i in range(ord[0], ord[1])]
...
def _load_results_to_net(net, model, t):
    ...
    # --- load ---
    net.res_load.p_mw = pd.Series([model.pD[d, t].value for d in net.load.index], index=net.load.index)
    net.res_load.p_mw *= model.baseMVA.value
    if 'AC' in model.name:
        # load
        net.res_load.q_mvar = pd.Series([model.qD[d, t].value for d in net.load.index], index=net.load.index)
...
def _sgen_results_to_net(net, model, t):
    ...
    for g in model.sG:
        net.res_sgen.iloc[g]['p_mw'] = model.psG[g, t].value * model.baseMVA.value

        if 'AC' in model.name:
            net.res_sgen.iloc[g]['q_mvar'] = model.qsG[g, t].value * model.baseMVA.value
...
def _gen_results_to_net(net, model, t):
    ...
    for g in model.gG:
        ...
        if 'AC' in model.name:
            net.res_gen.loc[g, 'q_mvar'] = model.qG[g, t].value * model.baseMVA.value
...
def _trafo_results_to_net(net, model, t):
    net.res_trafo.p_hv_mw = pd.Series([model.pThv[i, t].value for i in net.trafo.index], index=net.trafo.index)
    net.res_trafo.p_hv_mw *= model.baseMVA.value
    net.res_trafo.p_lv_mw = pd.Series([model.pTlv[i, t].value for i in net.trafo.index], index=net.trafo.index)
    ...
    if 'AC' in model.name:
        # transformer
        net.res_trafo.q_hv_mvar = pd.Series([model.qThv[i, t].value for i in net.trafo.index], index=net.trafo.index)
        net.res_trafo.q_hv_mvar *= model.baseMVA.value
        net.res_trafo.q_lv_mvar = pd.Series([model.qTlv[i, t].value for i in net.trafo.index], index=net.trafo.index)
    ...
    if hasattr(model, 'Tap_pos'):
        net.res_trafo['tap'] = pd.Series([model.Tap[i, t].value for i in net.trafo.index], index=net.trafo.index)
        net.res_trafo['tap_pos'] = pd.Series([model.Tap_pos[i, t].value for i in net.trafo.index], index=net.trafo.index)
...
def _shunt_results_to_net(net, model, t):
    for s in model.SHUNT:
        net.res_shunt.loc[s, 'p_mw'] = model.GB[s, t] * net.res_bus.vm_pu[net.shunt['bus'][s]] ** 2 * model.baseMVA.value

    if 'AC' in model.name:
        net.res_shunt.vm_pu = pd.Series(net.res_bus.vm_pu[net.shunt.bus].values, net.shunt.index)
        for s in model.SHUNT:
            net.res_shunt.loc[s, 'q_mvar'] = -model.BB[s, t] * net.res_bus.vm_pu[
