"""Wind power mix-in: attaches wind generator sets, parameters, and Q-control constraints to a multi-period model."""
...
class Windpower_multi_period(Sgens_multi_period):
    """Multi-period wind generator device module, extending sgen with Q-control and HC support."""
    ...
    def get_all(self, model):
        """No-op placeholder; wind generators are initialised via get_all_opf."""
    ...
    def _calc_wind_opf_parameters(self, model, SWmax=10000, SWmin=0):
        """Compute SWmax/SWmin limits and Q(U) slope parameters for HC wind generators."""
    ...
    def static_generation_wind_var_q(self, net):
        """Compute Q(P) and Q(U) characteristic slopes and populate static_generation_data limits."""
        ...
        self.q_limit_parameter = pd.DataFrame(
            {'m_qv': m, 'b_qv_min': b[:, 0], 'b_qv_max': b[:, 1],
             'm_qp_max': m_qp_max, 'm_qp_min': m_qp_min,
        ...
        if 'var_q' in self.net.sgen:
            ...
            self.static_generation_data['max_q'][sgens_var_q] = [
                y[0, int(self.static_generation_data.var_q[g])] * self.static_generation_data['p_inst'][g]
                for g in sgens_var_q
            ]
            self.static_generation_data['min_q'][sgens_var_q] = [
                y[1, int(self.static_generation_data.var_q[g])] * self.static_generation_data['p_inst'][g]
                for g in sgens_var_q
            ]
    ...
    def get_constraints(self, model, net):
        ...
        def QV_min(model, w):
            for (g, b) in model.sGbs:
                if g == w:
                    return (model.qsG[w] >= (self.q_limit_parameter.m_qv[model.var_q[w]] * model.v[b]
                                             + self.q_limit_parameter.b_qv_min[model.var_q[w]]) * model.PsG_inst[w])
        ...
        def QV_max(model, w):
            for (g, b) in model.sGbs:
                if g == w:
                    return (model.qsG[w] <= (self.q_limit_parameter.m_qv[model.var_q[w]] * model.v[b]
                                             + self.q_limit_parameter.b_qv_max[model.var_q[w]]) * model.PsG_inst[w])
    ...
    def unfix_variables(self, model):
        """Unfix real and reactive power variables for all HC wind generators."""
