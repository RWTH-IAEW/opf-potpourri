...
ev_obj = next(f for f in opf.flexibilities
              if f.__class__.__name__ == 'EV_multi_period')
...
for t in list(opf.model.T)[::4]:   # every hour
    ...
    p_opt = sum(pe.value(opf.model.p_opf[v, t]) * baseMVA
                for v in opf.model.veh)

    # Reference: use the fixed uncontrolled profile from p_req_ev if available
    p_ref = sum(pe.value(opf.model.p_req_ev[v, t]) * baseMVA
                for v in opf.model.veh
...
header = f"{'hour':>6}" + "".join(f"  {'veh'+str(v):>7}" for v in vehs_to_show)
print(header)
...
# Optimised cost
cost_opt = sum(
    pe.value(opf.model.p_opf[v, t]) * baseMVA
...
if cost_ref > 0:
    print(f"Cost saving:                €{cost_ref - cost_opt:.2f}  "
