...
print("  " + ", ".join(f"bus {b}" for _, b in inactive_wind[:10]),
...
for swmin_pu in swmin_values:
    ...
    n_active  = sum(1 for w in hc_sweep.model.WIND_HC
                    if (v := pe.value(hc_sweep.model.y[w])) and v > 0.5)
    total_mw  = sum(pe.value(hc_sweep.model.psG[w]) * baseMVA
...
print(f"{'eps':>6}  {'Active sites':>14}  {'Total wind (MW)':>17}  {'Losses (MW)':>13}")
for eps in eps_values:
    ...
    n_active = sum(1 for w in hc_w.model.WIND_HC
                   if (v := pe.value(hc_w.model.y[w])) and v > 0.5)
    total_mw = sum(pe.value(hc_w.model.psG[w]) * baseMVA
                   for w in hc_w.model.WIND_HC
                   if (v := pe.value(hc_w.model.y[w])) and v > 0.5)
    losses   = sum((pe.value(hc_w.model.pLfrom[l]) + pe.value(hc_w.model.pLto[l])) * baseMVA
